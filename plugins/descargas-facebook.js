...
let fkontak = { "key": { "participants":"0@s.whatsapp.net", "remoteJid": "status@broadcast", "fromMe": false, "id": "Halo" }, "message": { "contactMessage": { "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:y\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD` }}, "participant": "0@s.whatsapp.net" }
  if (!args[0]) throw `*🌸𝑀𝐴𝐾𝐼𝑀𝐴-𝐵𝛩𝑇🌸*\n\n*[ 🌸 ] هاذا الامر لتحميل من فيسبوك*\n\n*مثال*${usedPrefix + command} https://wa.me/967733707084*`;
  ...
  try {
    ...
    conn.sendFile(m.chat, r2es, 'error.mp4', '*🌸𝑀𝐴𝐾𝐼𝑀𝐴-𝐵𝛩𝑇🌸*\n',fkontak ,m);
  } catch (err1) {
    try {
      ...
      try {
        ...
        conn.sendFile(m.chat, VIDEO, 'error.mp4', `*🌸𝑀𝐴𝐾𝐼𝑀𝐴-𝐵𝛩𝑇🌸*\n`,fkontak ,m);
      } catch (err2) {
        try {
          const ress = await fg.fbdl(args[0]);
          const urll = await ress.data[0].url;
          await conn.sendFile(m.chat, urll, 'error.mp4','*🌸𝑀𝐴𝐾𝐼𝑀𝐴-𝐵𝛩𝑇🌸*\n',fkontak ,m);
        } catch (err3) {
          try {
            const res = await fbDownloader(args[0]);
            for (const result of res.download) {
              const ur = result.url;
              await conn.sendFile(m.chat, ur, 'error.mp4', '*🌸𝑀𝐴𝐾𝐼𝑀𝐴-𝐵𝛩𝑇🌸*\n',fkontak ,m);
            }
          } catch (err4) {
            try {
              ...
              await conn.sendFile(m.chat, url3, 'error.mp4', '*🌸𝑀𝐴𝐾𝐼𝑀𝐴-𝐵𝛩𝑇🌸*\n',fkontak ,m);
            } catch (err5) {
              try {
                const {result} = await facebookdl(args[0]).catch(async (_) => await facebookdlv2(args[0])).catch(async (_) => await savefrom(args[0]));
                for (const {url, isVideo} of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, '*🌸𝑀𝐴𝐾𝐼𝑀𝐴-𝐵𝛩𝑇🌸*\n',fkontak ,m);
              } catch (err6) {
                throw `*𝑀𝐴𝐾𝐼𝑀𝐴-𝐵𝛩𝑇*\n\n*[ 🌸 ] حصل خطاء*`;
              }
            }
...
handler.command = /^(فايسبوك|fb|فيس|fbdl|facebook|فيسبوك|facebookdl2|fbdl2|facebook3|fb3|facebookdl3|fbdl3|facebook4|fb4|facebookdl4|fbdl4|facebook5|fb5|facebookdl5|fbdl5)$/i;
